...
function UsersTab() {
  ...
  return (
    <TableContainer sx={{ boxShadow: 3, maxWidth: '100%', overflowX: 'auto' }} component={Paper}>
  <Typography variant="h5" sx={{ textAlign: 'center', my: 2 }}>Nos Membres</Typography>
  <Table>
    <TableHead sx={{ backgroundColor: '#728996' }}>
      <TableRow>
        {['Rôle', 'Prénom', 'Nom', 'Téléphone', 'Email', 'Réplique'].map((header) => (
          <TableCell key={header} align="center" sx={{ color: '#fff', fontWeight: 'bold', letterSpacing: 0.5 }}>
            {header}
          </TableCell>
        ))}
      </TableRow>
    ...
    <TableBody>
      {Array.isArray(users) && users.length === 0 ? (
        <TableRow>
          <TableCell colSpan={6} align="center">Aucun utilisateur trouvé.</TableCell>
        </TableRow>
      ) : (
        Array.isArray(users) &&
        users.map((user) => (
          <TableRow key={user._id}>
            <TableCell align="center">
              <span style={{
                backgroundColor: roleColors[user.role] || roleColors.default,
                color: 'white',
                padding: '4px 8px',
                borderRadius: 5,
                fontSize: '0.8rem',
            ...
            <TableCell align="center" sx={{ textTransform: 'capitalize' }}>{user.firstname}</TableCell>
