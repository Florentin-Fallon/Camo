  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 201) {
        setSnackbar({ open: true, message: `Bienvenue ${firstname} ${lastname} !`, severity: 'success' });
        setTimeout(() => navigate('/connexion'), 1500);
      }
    } catch (error) {
      setSnackbar({ open: true, message: 'Erreur lors de l\'inscription.', severity: 'error' });
    }
  };
  ...
  return (
    <Box
      sx={{
        ...
        height: '100vh',
        backgroundImage: 'url("https://images.unsplash.com/photo-1599397976795-4f638f02a5ef?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.0.3")',
        backgroundSize: 'cover',
        padding: 2,
        overflowX: 'hidden'
      }}
    >
      <Card sx={{ width: { xs: '80%', sm: '40%', md: '20%', lg: '20%' }, padding: 1, boxShadow: 3, borderRadius: 5 }}>
        <CardContent>
